...
export default function BillPaymentReportPage() {
  const [sidebarOpen, setSidebarOpen] = useState(true)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [data, setData] = useState<BillPaymentPayload | null>(null)

  const loadReport = useCallback(async () => {
    setLoading(true)
    setError(null)
    try {
      const res = await fetch('/api/accounts/budget-book/bill-payment-report', { cache: 'no-store' })
      const json = await res.json().catch(() => ({})) as { success?: boolean; data?: BillPaymentPayload; message?: string }
  ...
  const exportCsv = useCallback(() => {
    ...
    const lines: string[] = [
      ['Financial Year', 'Budget Code', 'Account Head Name', 'Total Budget Amount', 'Payable Expenditure', 'Budget Balance'].map(csvEscape).join(','),
      ...data.rows.map(row => [row.finYear, row.acSubhead, row.acSubheadName || '', row.totalBudgetAmount, row.totalExpenditureAmount, row.budgetBalanceAmount].map(csvEscape).join(',')),
      ['', '', 'GRAND TOTAL', data.grandTotalBudget, data.grandTotalExpenditure, data.grandTotalBalance].map(csvEscape).join(','),
    ]
    const blob = new Blob(['\uFEFF' + lines.join('\r\n')], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `bill-payment-report-${new Date().toISOString().slice(0, 10)}.csv`
  ...
  const exportExcel = useCallback(async () => {
    ...
    const sheetRows = [
      ['Financial Year', 'Budget Code', 'Account Head Name', 'Total Budget Amount', 'Payable Expenditure', 'Budget Balance'],
      ...data.rows.map(row => [row.finYear, row.acSubhead, row.acSubheadName || '', row.totalBudgetAmount, row.totalExpenditureAmount, row.budgetBalanceAmount]),
      ['', '', 'GRAND TOTAL', data.grandTotalBudget, data.grandTotalExpenditure, data.grandTotalBalance],
    ]
    const wb = XLSX.utils.book_new()
    const ws = XLSX.utils.aoa_to_sheet(sheetRows)
    ws['!cols'] = [{ wch: 14 }, { wch: 14 }, { wch: 58 }, { wch: 22 }, { wch: 22 }, { wch: 18 }]
    XLSX.utils.book_append_sheet(wb, ws, 'Bill Payment Report')
    XLSX.writeFile(wb, `bill-payment-report-${new Date().toISOString().slice(0, 10)}.xlsx`)
  }, [data])

  const rowCountLabel = useMemo(() => data ? `${data.rows.length} rows` : '0 rows', [data])
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f0f4f8', fontFamily: "'Segoe UI', 'Noto Sans Devanagari', sans-serif" }}>
      ...
      <main style={{ flex: 1, minWidth: 0, height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <nav className="dash-breadcrumb" aria-label="Breadcrumb">
          <Link href="/" className="dash-breadcrumb-home"><i className="bi bi-house-door-fill" aria-hidden="true" /> Home</Link>
          ...
          <span className="dash-breadcrumb-sep" aria-hidden="true">›</span>
          <span className="dash-breadcrumb-current">Bill Payment Report</span>
        </nav>
        ...
        <div style={{
          ...
          borderRadius: 16, marginBottom: 14, boxShadow: '0 4px 20px rgba(26,58,92,0.30)',
        }}>
          <div style={{ width: 50, height: 50, borderRadius: 12, background: 'rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <i className="bi bi-receipt-cutoff" style={{ fontSize: '1.5rem' }} />
          </div>
          <div>
            <div style={{ fontSize: '0.78rem', opacity: 0.85, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              Accounts Department
            </div>
            <h1 style={{ margin: 0, fontSize: '1.35rem', fontWeight: 800 }}>Bill Payment Report</h1>
            <div style={{ fontSize: '0.88rem', opacity: 0.9 }}>
              Account head-wise proposed &amp; payable amounts based on regular bills (1-Apr to current date).
            </div>
          </div>
          <button
            type="button"
            style={{ marginLeft: 'auto', background: 'rgba(255,255,255,0.16)', border: '1px solid rgba(255,255,255,0.3)', color: '#fff', borderRadius: 8, padding: '6px 11px', cursor: 'pointer', fontSize: '0.85rem' }}
        ...
        <div className="filters-panel no-print">
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'end', gap: 12 }}>
            ...
            <button type="button" style={ACTION_EXCEL_STYLE} onClick={exportExcel} disabled={!data || data.rows.length === 0}><i className="bi bi-file-earmark-excel me-1" /> Export Excel (.xlsx)</button>
            <button type="button" style={ACTION_CSV_STYLE} onClick={exportCsv} disabled={!data || data.rows.length === 0}><i className="bi bi-file-earmark-spreadsheet me-1" /> Export CSV</button>
            <button type="button" style={ACTION_PRINT_STYLE} onClick={() => window.print()} disabled={!data || data.rows.length === 0}><i className="bi bi-printer me-1" /> Print A4 / Save PDF</button>
          </div>
          <div style={{ marginTop: 10, display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
            <span style={{ background: '#ecfdf3', color: '#1f5137', border: '1px solid #ccefd9', borderRadius: 999, padding: '4px 10px', fontSize: '0.75rem', fontWeight: 700 }}>{rowCountLabel}</span>
          </div>
        </div>

        {!loading && !error && data && (
          <div style={KPI_GRID_STYLE}>
            <div style={KPI_CARD_STYLE}><div style={KPI_LABEL_STYLE}>Budget Heads</div><div style={KPI_VALUE_STYLE}>{data.rows.length}</div></div>
            <div style={KPI_CARD_STYLE}><div style={KPI_LABEL_STYLE}>Budget Provision</div><div style={KPI_VALUE_STYLE}>{fmtCompactInr(data.grandTotalBudget)}</div></div>
            <div style={KPI_CARD_STYLE}><div style={KPI_LABEL_STYLE}>Payable Expenditure</div><div style={KPI_VALUE_STYLE}>{fmtCompactInr(data.grandTotalExpenditure)}</div></div>
            <div style={KPI_CARD_STYLE}><div style={KPI_LABEL_STYLE}>Budget Balance</div><div style={KPI_VALUE_STYLE}>{fmtCompactInr(data.grandTotalBalance)}</div></div>
          </div>
        )}
        </div>

        <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', overflowX: 'hidden', overscrollBehavior: 'contain', padding: '12px 24px 28px 24px' }}>
        <section className="card p-3 p-md-4 shadow-sm border-0" id="bill-payment-report-sheet" style={{ borderRadius: 12, background: '#fff' }}>
          ...
          {!loading && !error && data && (
            <>
              <div className="table-responsive" style={{ maxHeight: '58vh', overflow: 'auto', border: '1px solid #e0eaf2', borderRadius: 10 }}>
                <table className="table table-bordered align-middle mb-0 report-table" style={{ minWidth: 1000 }}>
                  <thead className="table-light">
                    <tr>
                      ...
                      <th className="text-end" style={{ minWidth: 175, position: 'sticky', top: 0, zIndex: 5, background: '#f8fafc' }}>Total Budget Amount</th>
                      <th className="text-end" style={{ minWidth: 175, position: 'sticky', top: 0, zIndex: 5, background: '#f8fafc' }}>Payable Expenditure</th>
                      <th className="text-end" style={{ minWidth: 160, position: 'sticky', top: 0, zIndex: 5, background: '#f8fafc' }}>Budget Balance</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.rows.length === 0 && (
                      <tr><td colSpan={6} className="text-center text-muted py-4">No records found for the selected period.</td></tr>
                    )}
      ...
      <style jsx global>{`
        ...
        @media print {
          ...
          #bill-payment-report-sheet, #bill-payment-report-sheet * { visibility: visible !important; }
          #bill-payment-report-sheet { position: absolute; left: 0; top: 0; width: 100%; box-shadow: none !important; }
          .no-print { display: none !important; }
          .report-table th, .report-table td { font-size: 9pt !important; padding: 5px 7px !important; }
