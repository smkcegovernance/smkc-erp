...
const READONLY_STYLE: React.CSSProperties = {
  ...INPUT_STYLE, background: '#f0f6fb', color: '#3d5166', cursor: 'not-allowed',
}

// ── Inner component (uses useSearchParams) ─────────────────────────────────────

function AccountRemarkInner() {
  ...
  const loadProposal = useCallback(async () => {
    ...
    setLoadError('')
    try {
      const params = new URLSearchParams({ orderNo, type, remarkType: 'account' })
  ...
  return (
    <>
      ...
      <style>{`
        ...
        @media screen {
          #ga-account-remark-print-slip { display: none; }
        }
      `}</style>

      {/* ── Print slip (only visible during print) ── */}
      {data && (
        <div id="ga-account-remark-print-slip">
      ...
      <div style={{ flex: 1, padding: '28px 24px', minWidth: 0 }}>
        ...
        <nav className="dash-breadcrumb" aria-label="Breadcrumb">
          ...
          <span className="dash-breadcrumb-sep" aria-hidden="true">›</span>
          <span className="dash-breadcrumb-current">लेखा अभिप्राय</span>
        </nav>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              ...
              <p style={{ margin: 0, opacity: 0.85, fontSize: '0.88rem' }}>
                {data ? `प्रस्ताव #${data.orderNo} — ${data.finYear}` : 'कार्य प्रस्तावावरील लेखा अभिप्राय'}
              </p>
            </div>
            <button
              type="button"
        ...
        {loading ? (
          ...
          <div style={{ background: '#fff', borderRadius: 14, boxShadow: '0 2px 16px rgba(18,49,76,0.08)', overflow: 'hidden' }}>
            ...
            {/* Fixed info band */}
            <div style={{ background: '#eaf3fb', borderBottom: '1.5px solid #b6d0e8', padding: '18px 28px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px 24px' }}>
                <div>
                  <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#7fb3d3', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 3 }}>विभागाचे नाव</div>
                  <div style={{ fontSize: '0.95rem', fontWeight: 700, color: '#1a5276' }}>{data.deptName || '—'}</div>
                </div>
                <div>
                  <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#7fb3d3', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 3 }}>नस्ती क्रमांक</div>
                  <div style={{ fontSize: '0.95rem', fontWeight: 700, color: '#1a5276' }}>{data.finYear} / {data.nastiNo}</div>
                </div>
                <div>
                  <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#7fb3d3', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 3 }}>प्रकार</div>
                  <div>
              ...
              <div style={{ marginTop: 10 }}>
                <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#7fb3d3', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 3 }}>कामगिरीचे नाव</div>
                <div style={{ fontSize: '1rem', fontWeight: 700, color: '#18324a' }}>{data.workName || '—'}</div>
              </div>
            </div>

            {/* Standard certification text */}
            <div style={{ padding: '16px 28px', background: '#f7fafd', borderBottom: '1px solid #e0eaf2' }}>
              <p style={{ margin: 0, fontSize: '0.88rem', color: '#3d5166', lineHeight: 1.7, fontFamily: "'Noto Sans Devanagari', sans-serif" }}>
                सादर प्रस्तावाची छाननी करण्यात आली असून लेखाशीर्ष प्रस्तावित कामगिरीसाठी अनुज्ञेय आहे.
            ...
            <div style={{ padding: '28px 28px 32px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px 32px' }}>
                ...
                {/* AC Head (read-only — show code + name) */}
...
export default function AccountRemarkPage() {
  return (
    <Suspense fallback={
