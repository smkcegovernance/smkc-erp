...
function SectionLabel({ text }: { text: string }) {
  return (
    <div style={{ borderLeft: '4px solid #1a6db5', paddingLeft: 12, marginBottom: 16, marginTop: 28 }}>
      <span style={{ fontSize: '0.78rem', fontWeight: 700, color: '#1a6db5', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
        {text}
      </span>
    </div>
  )
            ...
            {filtered.length === 0 ? (
              ...
              <li key={`${o.value}-${i}`}>
                <button type="button" onClick={() => select(o.value)}
                  style={{ display: 'block', width: '100%', textAlign: 'left', padding: '8px 14px', border: 'none', cursor: 'pointer', fontSize: '0.875rem', background: o.value === value ? 'rgba(26,109,181,0.08)' : 'transparent', color: o.value === value ? '#1a6db5' : '#18324a', fontWeight: o.value === value ? 600 : 400 }}
                  onMouseEnter={e => { if (o.value !== value) (e.currentTarget as HTMLButtonElement).style.background = '#f0f6fc' }}
                  onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = o.value === value ? 'rgba(26,109,181,0.08)' : 'transparent' }}
                >{o.label}</button>
              </li>
            ))}
          </ul>
  ...
  return (
    <div style={OVERLAY_STYLE} onClick={saving ? undefined : onCancel}>
      <div style={MODAL_STYLE} onClick={e => e.stopPropagation()}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #e0eaf2', background: 'linear-gradient(135deg, #1a6db5 0%, #114d8a 100%)', borderRadius: '14px 14px 0 0', color: '#fff', display: 'flex', alignItems: 'center', gap: 12 }}>
          <i className="bi bi-shield-exclamation" style={{ fontSize: '1.4rem' }} />
          <div>
            <div style={{ fontSize: '0.75rem', opacity: 0.8 }}>पुष्टी करा</div>
            <h4 style={{ margin: 0, fontSize: '1rem', fontWeight: 700 }}>निविदा प्रस्ताव जतन करायचा आहे?</h4>
          </div>
        </div>
        ...
        <div style={{ padding: '12px 20px 18px', display: 'flex', gap: 10, justifyContent: 'flex-end', borderTop: '1px solid #f0f4f8' }}>
          ...
          <button type="button" disabled={saving} onClick={onConfirm}
            style={{ padding: '7px 20px', borderRadius: 7, border: 'none', background: '#1a6db5', color: '#fff', fontWeight: 700, fontSize: '0.84rem', minWidth: 160, cursor: saving ? 'not-allowed' : 'pointer', boxShadow: '0 2px 6px rgba(26,109,181,0.25)', display: 'flex', alignItems: 'center', gap: 8 }}>
            {saving
  ...
  return (
    <div>
      <div style={{ marginBottom: 6, display: 'flex', alignItems: 'center', gap: 10 }}>
        <button type="button" onClick={toggleAll} style={{ fontSize: '0.78rem', padding: '3px 10px', borderRadius: 6, border: '1.5px solid #1a6db5', background: selected.length === ALL_WARDS.length ? '#1a6db5' : '#fff', color: selected.length === ALL_WARDS.length ? '#fff' : '#1a6db5', fontWeight: 600, cursor: 'pointer' }}>
          {selected.length === ALL_WARDS.length ? 'सर्व काढा' : 'सर्व निवडा'}
        </button>
        {selected.length > 0 && <span style={{ fontSize: '0.78rem', color: '#1a6db5', fontWeight: 600 }}>{selected.length} प्रभाग निवडले</span>}
      </div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
        {ALL_WARDS.map(w => (
          <button key={w} type="button" onClick={() => toggle(w)}
            style={{ width: 40, height: 32, borderRadius: 7, border: `1.5px solid ${selected.includes(w) ? '#1a6db5' : '#c8d8e8'}`, background: selected.includes(w) ? '#1a6db5' : '#fff', color: selected.includes(w) ? '#fff' : '#5e7388', fontSize: '0.82rem', fontWeight: selected.includes(w) ? 700 : 400, cursor: 'pointer' }}>
            {w}
          </button>
        ))}
      </div>
...
export default function WorkProposalOver10LPage() {
  ...
  const loadNastiNo = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/general-administration/work-proposals/next-sequence?type=T')
  ...
  const loadBudget = useCallback((dc: string, ac: string, fy: string) => {
    ...
    fetch(`/api/general-administration/work-proposals/budget?deptCode=${encodeURIComponent(dc)}&acSubhead=${encodeURIComponent(ac)}&finYear=${encodeURIComponent(fy)}&proposalType=T`)
  ...
  const canSubmit =
    ...
    workAmountNum > 0 && !under1L && !budgetExceeded &&
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (under1L) {
      setClientError('निविदा रक्कम ₹1 लाखापेक्षा कमी असू नये. कृपया ₹1 लाखापर्यंतच्या प्रस्तावासाठी दरपत्रक फॉर्म वापरा.')
      return
    }
  ...
  async function doSave() {
    ...
    try {
      ...
      const res = await fetch('/api/general-administration/work-proposals/over-10l', {
      ...
      if (res.ok && json.success) {
        ...
        // Capture print data BEFORE reset
        ...
        const pd: WorkProposalPrintData = {
        ...
        loadNastiNo()
        setPopup({ open: true, tone: 'success', title: 'प्रस्ताव यशस्वीरित्या जतन झाला', description: `निविदा क्रमांक: ${json.orderNo ?? '—'}` })
      } else {
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f0f4f8', fontFamily: "'Segoe UI', 'Noto Sans Devanagari', sans-serif" }}>
      ...
      <div style={{ flex: 1, padding: '28px 24px', minWidth: 0 }}>
        ...
        <nav className="dash-breadcrumb" aria-label="Breadcrumb">
          ...
          <span className="dash-breadcrumb-sep" aria-hidden="true">›</span>
          <span className="dash-breadcrumb-current">निविदा मागवण्यास मान्यता (₹10 लाखांवर)</span>
        </nav>

        {/* Header */}
        <div style={{ background: 'linear-gradient(135deg, #1a6db5 0%, #114d8a 100%)', borderRadius: 16, padding: '22px 28px', marginBottom: 24, color: '#fff', boxShadow: '0 4px 20px rgba(26,109,181,0.3)' }}>
        ...
        {/* Success banner */}
        {savedOrderNo !== null && (
          <div style={{ background: '#e8f3ff', border: '1.5px solid #1a6db5', borderRadius: 10, padding: '12px 18px', marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <i className="bi bi-check-circle-fill" style={{ color: '#1a6db5', fontSize: '1.3rem' }} />
              <span style={{ color: '#114d8a', fontWeight: 600 }}>
                प्रस्ताव यशस्वीरित्या नोंदवला! निविदा क्रमांक: <strong>#{savedOrderNo}</strong>
              </span>
            </div>
            {lastPrintData && (
              <button
                ...
                style={{ padding: '7px 16px', borderRadius: 7, border: 'none', background: '#1a6db5', color: '#fff', fontWeight: 700, fontSize: '0.84rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7 }}
              >
        ...
        <div style={{ background: '#fff', borderRadius: 14, boxShadow: '0 2px 16px rgba(18,49,76,0.08)', padding: '28px 32px' }}>
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
              ...
              <div>
                <label style={LABEL_STYLE}>आर्थिक वर्ष <span style={{ color: '#1a6db5' }}>*</span></label>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={LABEL_STYLE}>विभागाचे नांव <span style={{ color: '#1a6db5' }}>*</span></label>
              {loadingDepts
              ...
              <div style={{ position: 'relative' }}>
                ...
                {loadingNasti && (
                  <span style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)' }}>
                    <span className="spinner-border spinner-border-sm" style={{ color: '#1a6db5' }} role="status" />
                  </span>
                )}
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={LABEL_STYLE}>कामगिरीचे नाव <span style={{ color: '#1a6db5' }}>*</span></label>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={LABEL_STYLE}>कामगिरीचे ठिकाण <span style={{ color: '#1a6db5' }}>*</span></label>
            ...
            <div style={{ marginBottom: 20, maxWidth: 400 }}>
              <label style={LABEL_STYLE}>कामगिरीचा स्थल नकाशा सोबत जोडला आहे का? <span style={{ color: '#1a6db5' }}>*</span></label>
              <AaheNahiSelect value={mapAttached} onChange={setMapAttached} />
            </div>

            <div style={{ marginBottom: 20 }}>
              <label style={LABEL_STYLE}>वॉर्ड क्रमांक <span style={{ color: '#1a6db5' }}>*</span></label>
              <WardPicker selected={wards} onChange={setWards} />
            </div>

            <div style={{ marginBottom: 20 }}>
              <label style={LABEL_STYLE}>कामगिरीची आवश्यकता <span style={{ color: '#1a6db5' }}>*</span></label>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={LABEL_STYLE}>यापूर्वी ही कामगिरी कधी करण्यात आलेली होती का? <span style={{ color: '#1a6db5' }}>*</span></label>
            ...
            <div style={{ marginBottom: 20, maxWidth: 360 }}>
              <label style={LABEL_STYLE}>कामगिरीसाठी अपेक्षित खर्च रुपये <span style={{ color: '#1a6db5' }}>*</span></label>
            ...
            <div style={{ marginBottom: 20, maxWidth: 400 }}>
              <label style={LABEL_STYLE}>अंदाजपत्रकास सक्षम प्राधिकाऱ्याची तांत्रीक मान्यता प्राप्त आहे का? <span style={{ color: '#1a6db5' }}>*</span></label>
              <HoNahiSelect value={techApproval} onChange={setTechApproval} />
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
              <div>
                <label style={LABEL_STYLE}>तांत्रीक मान्यता क्रमांक <span style={{ color: '#1a6db5' }}>*</span></label>
              ...
              <div>
                <label style={LABEL_STYLE}>तांत्रीक मान्यता दिनांक <span style={{ color: '#1a6db5' }}>*</span></label>
                <input type="date" value={techSanctionDate} onChange={e => setTechSanctionDate(e.target.value)} style={INPUT_STYLE} />
              </div>
            </div>

            <div style={{ marginBottom: 20, maxWidth: 400 }}>
              <label style={LABEL_STYLE}>अंदाजपत्रक DSR दरांप्रमाणे नसल्यास सक्षम प्राधिकाऱ्याची मंजुरी आहे का? <span style={{ color: '#1a6db5' }}>*</span></label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
              <div>
                <label style={LABEL_STYLE}>प्रस्तावित कामगिरीची जागा महानगरपालिकेच्या मालकीची व प्रत्यक्ष ताब्यात आहे का? <span style={{ color: '#1a6db5' }}>*</span></label>
                <AaheNahiSelect value={placeOwnership} onChange={setPlaceOwnership} />
              </div>
              <div>
                <label style={LABEL_STYLE}>जागा मालकी हक्क/ना हरकत दाखला याबाबतचे दस्तऐवज सोबत जोडले आहेत का? <span style={{ color: '#1a6db5' }}>*</span></label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
              <div>
                <label style={LABEL_STYLE}>मालकी हक्क असणाऱ्या व्यक्ती/संस्थेचे ना हरकत प्रमाणपत्र घेतले आहे का? <span style={{ color: '#1a6db5' }}>*</span></label>
                <AaheNahiSelect value={nocCertificate} onChange={setNocCertificate} />
              </div>
              <div>
                <label style={LABEL_STYLE}>प्रस्तावित कामगिरीच्या जागेबद्दल काही वाद विवाद आहेत का? <span style={{ color: '#1a6db5' }}>*</span></label>
                <AaheNahiSelect value={anyDispute} onChange={setAnyDispute} />
              </div>
            </div>

            <div style={{ marginBottom: 20, maxWidth: 400 }}>
              <label style={LABEL_STYLE}>प्रस्तावित कामगिरीच्या जागेबद्दल काही न्यायालयीन प्रकरण आहे का? <span style={{ color: '#1a6db5' }}>*</span></label>
              <AaheNahiSelect value={courtCase} onChange={setCourtCase} />
            </div>

            <div style={{ marginBottom: 20 }}>
              <label style={LABEL_STYLE}>न्यायालयीन प्रकरण दाखल असल्यास त्याचा तपशील <span style={{ color: '#1a6db5' }}>*</span></label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
              <div>
                <label style={LABEL_STYLE}>बांधकाम प्रस्तावित असल्यास नगररचना विभागाकडून अनुज्ञेयता तपासली आहे का? <span style={{ color: '#1a6db5' }}>*</span></label>
                <AaheNahiSelect value={townPlanCheck} onChange={setTownPlanCheck} />
              </div>
              <div>
                <label style={LABEL_STYLE}>बांधकाम नकाशांना नगररचना विभागाची परवानगी घेतली आहे का? <span style={{ color: '#1a6db5' }}>*</span></label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
              <div>
                <label style={LABEL_STYLE}>प्रस्तावित खर्च महानगरपालिका अधिनियम व शासन निर्देशांप्रमाणे अनुज्ञेय आहे का? <span style={{ color: '#1a6db5' }}>*</span></label>
                <AaheNahiSelect value={expendValid} onChange={setExpendValid} />
              </div>
              <div>
                <label style={LABEL_STYLE}>वस्तूंची खरेदी असल्यास शिल्लक वस्तूंचा तक्ता सोबत जोडला आहे का? <span style={{ color: '#1a6db5' }}>*</span></label>
                <AaheNahiSelect value={stockListAttached} onChange={setStockListAttached} />
              </div>
            </div>

            <div style={{ marginBottom: 20, maxWidth: 400 }}>
              <label style={LABEL_STYLE}>प्रस्तावास कामगिरीपूर्वीचे फोटो जोडले आहेत का? <span style={{ color: '#1a6db5' }}>*</span></label>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={LABEL_STYLE}>अनुज्ञेय लेखाशीर्ष <span style={{ color: '#1a6db5' }}>*</span></label>
              {loadingSubheads
                ? <div style={{ ...INPUT_STYLE, color: '#9aabbf' }}><span className="spinner-border spinner-border-sm me-2" />लोड होत आहे...</div>
                : <SearchableSelect
            ...
            <div style={{ marginBottom: 20, maxWidth: 360 }}>
              <label style={LABEL_STYLE}>प्रस्तावित कामाचा खर्च (₹) <span style={{ color: '#1a6db5' }}>*</span></label>
              <input
                type="number" min={0} step="0.01" value={proposalCost}
                onChange={e => { setProposalCost(e.target.value); setClientError('') }}
                style={{ ...INPUT_STYLE, borderColor: (under1L || budgetExceeded) ? '#c0392b' : '#c8d8e8' }}
                placeholder="0.00 (किमान ₹1,00,001)"
              />
              {under1L && <p style={{ color: '#c0392b', fontSize: '0.8rem', marginTop: 4, marginBottom: 0 }}><i className="bi bi-exclamation-circle-fill me-1" />निविदा रक्कम ₹1 लाखापेक्षा जास्त असणे आवश्यक आहे</p>}
              {!under1L && budgetExceeded && budgetInfo && <p style={{ color: '#c0392b', fontSize: '0.8rem', marginTop: 4, marginBottom: 0 }}><i className="bi bi-shield-exclamation me-1" />बजेट मर्यादा ओलांडली! कमाल उपलब्ध: ₹ {budgetInfo.remainingBudget.toLocaleString('en-IN')}</p>}
            </div>

            <div style={{ marginBottom: 20, maxWidth: 400 }}>
              <label style={LABEL_STYLE}>प्रस्तावित लेखार्शीष प्रस्तावित कामगिरीसाठी अनुज्ञेय आहे का? <span style={{ color: '#1a6db5' }}>*</span></label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
              <div>
                <label style={LABEL_STYLE}>ही कामगिरी अन्य विभाग/योजनेतून प्रस्तावित आहे का? <span style={{ color: '#1a6db5' }}>*</span></label>
                <AaheNahiSelect value={otherDept} onChange={setOtherDept} />
              </div>
              <div>
                <label style={LABEL_STYLE}>सदर कामगिरी एका सलग कामाचे अनेक तुकडे करून पार पाडली जात आहे का? <span style={{ color: '#1a6db5' }}>*</span></label>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button type="submit" disabled={!canSubmit}
                style={{ padding: '10px 32px', borderRadius: 9, border: 'none', background: canSubmit ? '#1a6db5' : '#d6d6d6', color: '#fff', fontWeight: 700, fontSize: '0.95rem', cursor: canSubmit ? 'pointer' : 'not-allowed', boxShadow: canSubmit ? '0 2px 8px rgba(26,109,181,0.25)' : 'none', display: 'flex', alignItems: 'center', gap: 8 }}>
