...
function formatDate(value: string) {
  if (!value) return 'नोंद केलेले नाही'
...
function renderValue(value: string | boolean | null | undefined) {
  if (typeof value === 'boolean') return value ? 'होय' : 'नाही'
  if (!value || value.trim().length === 0) return 'नोंद केलेले नाही'
  return value
}
...
export default function PublicPrintableApplicationPage() {
  ...
  useEffect(() => {
    if (!snapshot || searchParams.get('autoprint') !== '1') return
      ...
      <div className={styles.shell}>
        <div className={styles.emptyState}>
          ...
          <div className={styles.toolbarActions}>
            <Link className={styles.toolbarLink} href="/public/disability-registration">
            ...
            <div className={styles.heroBlock}>
              <div className={styles.photoFrame}>
                {formData.photoPreview
                  ? <img src={formData.photoPreview} alt="Applicant" />
                  : <div className={styles.placeholder}>फोटो उपलब्ध नाही</div>
                }
              </div>
            ...
            <div className={styles.listCard}>
              ...
              <div className={styles.list}>
                {formData.disabilityTypes.length > 0
                  ? formData.disabilityTypes.map((item) => <span className={styles.chip} key={item}>{item}</span>)
            ...
            <div className={styles.listCard}>
              ...
              <div className={styles.list}>
                {formData.assistiveDevices.length > 0
                  ? formData.assistiveDevices.map((item) => <span className={styles.chip} key={item}>{item}</span>)
                  : <div className={styles.fieldValue}>नोंद केलेले नाही</div>
                }
              </div>
            </div>
